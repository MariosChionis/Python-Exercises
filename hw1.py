...
def f(x):
    ...
    return truediv(1,add(2,truediv(add(mul(3,x),add(x,4)),add(pow(x,2),mul(x,4)))))
...
def fix_me():
    ...
    from math import pi
    r=int(input())
    katheti_pleura=int(input())
    ipotinousa=sqrt(2*katheti_pleura**2)
    emv_kyk=pi*r**2
    perif_kyk=2*pi*r
    emv_orthgoniou=(katheti_pleura**2)*0.5
    perif_orthogoniou=katheti_pleura*2+ipotinousa
    print('Emvado kyklou aktinas',r, '=',emv_kyk,'me perifereia =',perif_kyk)
    print('Emvado isoskelous or8ogwniou trigwnou me ka8eti pleura',katheti_pleura,'=',emv_orthgoniou,'me perifereia =',perif_orthogoniou)
