...
def print_location(loc):
    ...
    print(loc[0],"({})".format(loc[3]),"at coordinates","{},".format(loc[1]),loc[2])
...
def nearest_location(loc, loc_list, loc_type=None):
    ...
    for x in range(len(loc_list)):
        if x==0:
            min=distance(loc,loc_list[0])
            locname=x
        if loc_type==None and loc!=loc_list[x]:
            if min>distance(loc,loc_list[x]):
                min=distance(loc,loc_list[x])
                locname=x
        else:
            if loc_type==type(loc_list[x]) and loc!=loc_list[x]:
                if min>distance(loc,loc_list[x]):
                    min=distance(loc,loc_list[x])
                    locname=x
    return loc_list[locname]
...
def pick_cherries(field):
    ...
    def flatten(li):
        return sum(([x] if not isinstance(x, list) else flatten(x)
            for x in li), [])
    list_of_lists=flatten(field)
    i=0
    while i<=len(list_of_lists)-1 and list_of_lists[i]!=None:
...
def all_lc(func, ls):
    ...
    (for 'h while)."""
    """GRAPSTE TON KWDIKA SAS APO KATW."""
    sum1=[True if func(x)==True else False for x in ls]
    x=all(sum1)
    if x==True:
        return True
    else:
...
def primes_up_to(n):
    ...
    prime = list(filter(lambda x:all(x % y !=0 for y in range(2,x)),range(2,n)))
    print(prime)
